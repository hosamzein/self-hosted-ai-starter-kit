...
3. For Open WebUI access, open http://localhost:3000 and configure the Ollama connection to use `host.docker.internal:11434`
...
2. Create an admin account on first visit (if ENABLE_SIGNUP is set to true in .env)
3. Start chatting with your local Ollama models immediately
4. The interface provides a ChatGPT-like experience with your self-hosted AI models

> [!TIP]
> Open WebUI automatically connects to Ollama running in the same Docker network. You can switch between different models and manage conversations with a familiar chat interface.
...
- To disable public signups after creating your admin account, set `ENABLE_SIGNUP=false` in your .env file and restart
- You can download additional models directly from the Open WebUI interface
- Chat history and settings are persisted in the `openwebui_storage` volume
